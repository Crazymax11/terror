...
Terror.prototype.logMultilineError = function(message, level, log) {
    ...
    _log(_level + ' ' + message.replace(NEW_LINE_RE, arrows));
...
Terror.prototype.getFullStack = function() {
    ...
    do {
        if (error.stack) {
            stack = String(error.stack).replace(NEW_LINE_RE, '\n' + indent) + '\n';
        } else {
...
Terror.prototype.setMessage = function(message) {
    var newMessage = isString(message) && message.indexOf('%') > -1 ? sPrintF(message, this.data) : message;

    if (newMessage !== this.message && this.stack && typeof this.stack.replace === 'function') {
        // @see https://nodejs.org/api/errors.html#errors_error_message
        // The message passed to the constructor
        // will also appear in the first line of the stack trace of the Error,
        // however changing this property after the Error object is created
        // may not change the first line of the stack trace
        // (for example, when error.stack is read before this property is changed).
        // nodejs@8 doesn't update message in error.stack
        this.stack = this.stack.replace(this.message, newMessage);
...
function sPrintF(str, data) {
    return str.replace(TEMPLATE_RE, function (match, variable) {
        return has.call(data, variable) ? data[variable] : match;
    });
}
